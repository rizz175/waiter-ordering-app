            ...
            return GetBuilder<OrderController>(builder: (orderController) {
              ...
              if(restController.restaurant != null && restController.restaurant.selfDeliverySystem == 1) {
               ...
               /* _deliveryCharge = orderController.distance * Get.find<SplashController>().configModel.perKmShippingCharge;
                ...
                double distaciaEs=orderController.distance;

                if(orderController.distance<=0.5){
                  _charge = 2.0;
                  _deliveryCharge = 2.0;
                }
                else if(orderController.distance<=1.5){
                  _charge = 4.0;
                  _deliveryCharge = 4.0;
                }
                else if(orderController.distance<=2.0){
                  _charge = 5.0;
                  _deliveryCharge = 5.0;
                }
                else if(orderController.distance<=2.5){
                  _charge = 6.0;
                  _deliveryCharge = 6.0;
                }
                else if(orderController.distance<=3.0){
                  _charge = 6.5;
                  _deliveryCharge = 6.5;
                }
                else{
